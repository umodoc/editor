...
4. Merged options створюються один раз через `getOptions()` і зберігаються як reactive ref
...
| Файл | Exports | Опис |
|------|---------|------|
| `options.js` | `getOptions(props, globalOptions)` | Merge props + global options + defaults через ObjectSchema |
