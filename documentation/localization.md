...
l(data)          // Locale-aware label: l({ en_US: 'Bold', zh_CN: '粗体' }) → 'Bold' або '粗体'
useI18n()        // Повний vue-i18n global instance (locale, mergeLocaleMessage тощо)
```

`l()` використовується для даних із конфігурації (dicts, webPages тощо), де label може бути string або `{ en_US, zh_CN }` object.
...
| Файл | Мова | Статус |
|------|------|--------|
| `src/locales/zh-CN.json` | Китайська (спрощена) | Повний (default) |
| `src/locales/en-US.json` | Англійська | Повний |
...
Тільки `zh-CN` та `en-US` вбудовані у i18n instance при ініціалізації. Інші мови — community contributions, потребують додаткового підключення.
...
Dicts та деякі інші опції використовують формат:
```js
{
...
Функція `l()` з `src/composables/i18n.js` автоматично вибирає правильну мову. Schema validation перевіряє, що label є або string, або object з ключами `en_US` / `zh_CN`.
...
3. Зареєструй у `src/i18n.js`:
   ...
   ```
4. Додай валідацію locale у `src/options/schema.js` (розшир список `['zh-CN', 'en-US', ...]`)
...
1. Знайди ключ у `src/locales/en-US.json` (або `zh-CN.json`)
2. Зміни значення
