<template>
    <menus-button :ico="'echarts'" :text="t('tools.echarts.text')" huge  @menu-click="menuClick">
        <modal :visible="dialogVisible" icon="echarts"
            :header="dialogAddOrEdit ? t('tools.echarts.add') : t('tools.echarts.edit')" width="960px"
            @confirm="setConfirm" @close="dialogVisible = false">
            <div class="umo-echarts-container">
                <div class="umo-echarts-header">
                    <t-radio-group v-model="baseModeSet" default-value="0" style="float: left;" v-if="modelMode==1">
                        <t-radio :value="0">{{t('tools.echarts.setting')}}</t-radio>
                        <t-radio :value="1">{{t('tools.echarts.dataGrid')}}</t-radio>
                    </t-radio-group>
                    <t-radio-group v-model="modelMode" default-value="0" style="float: right;">
                        <t-radio :value="1">{{t('tools.echarts.visualization')}}</t-radio>
                        <t-radio :value="0">{{t('tools.echarts.source')}}</t-radio>
                    </t-radio-group>
                </div>
                <div class="umo-echarts-sourceCenter" v-if="modelMode==0">
                    <t-textarea v-model="sourceOptions" class="umo-echarts-code" autofocus
                        :placeholder="t('tools.echarts.placeholder')" />
                    <div class="umo-echarts-render">
                        <div id="echartsSourceModeId" class="umo-echarts-svg"></div>
                    </div>
                </div>
                <div class="umo-echarts-sourceCenter" v-if="modelMode==1&&baseModeSet==0">
                    ...
                    <div class="umo-echarts-settting">
                        <t-form>
                            <!--图形类型-->
                            <t-form-item :label="t('tools.echarts.set.seriesType')" name="seriesType">
                                <t-select v-model="baseConfig.config.seriesType" class="demo-select-base">
                                    <t-option v-for="(item, index) in baseData.seriesType" :value="item.code"
                            ...
                            <t-form-item v-if="baseConfig.config.seriesType=='line'">
                                <!--平滑折线-->
                                <t-checkbox v-if="baseConfig.config.seriesType=='line'"
                                    v-model="baseConfig.config.smooth">{{t('tools.echarts.set.smooth')}}</t-checkbox>
                            </t-form-item>
                            <t-form-item>
                                <!--图例-->
                                <t-checkbox
                                    v-model="baseConfig.config.legend">{{t('tools.echarts.set.legend')}}</t-checkbox>
                            </t-form-item>
                            <!--图例布局-->
                            <t-form-item v-if="baseConfig.config.legend" :label="t('tools.echarts.set.legendorient')"
                                name="legendorient">
                                <t-select v-model="baseConfig.config.legendorient" class="demo-select-base">
                                    <t-option v-for="(item, index) in baseData.legendorient" :value="item.code"
                                        :label="item.name" :key="index">
                                        {{ item.name }}
                                    </t-option>
                                </t-select>
                            ...
                            <!--图例纵向位置-->
                            <t-form-item v-if="baseConfig.config.legend" :label="t('tools.echarts.set.legendlocation')"
                                name="legend">
                                <t-select v-model="baseConfig.config.legendlocation" class="demo-select-base">
                                    <t-option v-for="(item, index) in baseData.legendlocation" :value="item.code"
                                        :label="item.name" :key="index">
                                        {{ item.name }}
                                    </t-option>
                                </t-select>
                            ...
                            <!--图例横向位置-->
                            <t-form-item v-if="baseConfig.config.legend" :label="t('tools.echarts.set.legendleft')"
                                name="legendleft">
                                <t-select v-model="baseConfig.config.legendleft" class="demo-select-base">
                                    <t-option v-for="(item, index) in baseData.titleleft" :value="item.code"
                                        :label="item.name" :key="index">
                                        {{ item.name }}
                                    </t-option>
                                </t-select>
                            ...
                            <!--标题名称-->
                            <t-form-item :label="t('tools.echarts.set.titleText')" name="title">
                                <t-input v-model="baseConfig.config.titleText"
                                    :placeholder="t('tools.echarts.set.titleTextPlaceholder')" />
                            </t-form-item>
                            <!--标题位置-->
                            <t-form-item v-if="baseConfig.config.titleText!=''"
                                :label="t('tools.echarts.set.titleleft')" name="titleleft">
                                <t-select v-model="baseConfig.config.titleleft" class="demo-select-base">
                                    <t-option v-for="(item, index) in baseData.titleleft" :value="item.code"
                                        :label="item.name" :key="index">
                                        {{ item.name }}
                                    </t-option>
                ...
                <div class="umo-echarts-sourceCenter" v-if="modelMode==1&&baseModeSet==1">
                    <div class="umo-echarts-render" style="margin-left: 2px;">
                        <t-table id="echartsSettingGridId" class="umo-echarts-table" row-key="tabkey"
                            :columns="baseData.Columns" :data="baseConfig.data" :size="'small'" :showHeader="false"
                            :fixedRows="[1]" :editable-cell-state="editableCellState" bordered lazyLoad />
                    </div>
                </div>
            </div>
        </modal>
...
<script setup lang="ts">
    ...
    const setConfirm = () => {
        let resOptions = {
            id: "", name: "", mode: modelMode, chartOptions: null, chartConfig: null, describe: "", nodeAlign: "center",
            margin: {}
        }
        if (!dialogAddOrEdit) {
        ...
        if (options.value.echarts?.haveImage || resOptions.mode == 0) {
            //源码模式或havImage 
            var dataURI = (resOptions.mode == 1 ? settingMyChart : sourceMyChart).getDataURL({
    ...
    watch(
        ...
        ([newSourceOptions, newModelMode, newConfigData]: [any | null, Number, any | null, Number]) => {
            DisposeChart();
            loadModeEchart();
        },
    ...
    async function loadModeEchart() {
        ...
        if (typeof echarts !== 'undefined') {
            //  根据参数不同 实现效果不同
            DisposeChart()
            if (modelMode == 1) {//  实际的参数设置
                let newData = calbaseConfigData(baseConfig.data)
                if (!(newData == null || newData.length == 0)) {
                    let newOptions = calbaseConfigOptions(newData, baseConfig.config)
                    settingMyChart = echarts.init(document.getElementById("echartsSettingModeId"))
                    try {
                ...
                sourceMyChart = echarts.init(document.getElementById("echartsSourceModeId"))
                try {
                    let calOptions = normalizeJsonString(sourceOptions)
            ...
            baseData.Columns.push({
                ...
                edit: {
                    ...
                    onEdited: (context: any) => {
                        let newData = context.newRowData
                        if (context.rowIndex > 0) {
                            for (let artt1 in newData) {
    ...
    function LoadBaseConfig(cachebaseConfig: any) {
        ...
        if (cachebaseConfig.config != null) {
            for (let attr1 in cachebaseConfig.config) {
