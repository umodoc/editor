...
export function calbaseConfigOptions(data: any, config: any) {
    ...
    function calbaseConfigOptionsInlegend() {
        ...
        for (let i = 1; i < alphabet.length; i++) {
            let curColName = alphabet[i]
            if (data[0][curColName] == "" || data[0][curColName] == null) {
                continue
            }
    ...
    function calbaseConfigOptionsInType() {
        ...
        for (let i = 0; i < colNameList.length; i++) {
            ...
            //平滑折线
            if (config.smooth && resOption.series[resOption.series.length - 1].type == "line") {
    ...
    let newOptions = options.value.onCustomizeChartSettings?.(data, config)
